...
- **Write-volume envelope** — ✅ documented, not engineered around. `MAX_DOC_BYTES` (30 MiB) against a configurable 500ms–10s interval means a limit-sized document costs ~15 MiB/s at the 2s default; the relationship must be documented and the default justified. Pre-existing, not introduced here. Adaptive flushing explicitly not built (§XI). FR-010a, SC-020.
