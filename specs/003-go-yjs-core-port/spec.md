...
- **§IV (CRDT Correctness — One Core, Fuzz-Gated)** mandates the service build on `y-crdt` *by name* and carry exactly one core. The amendment must re-point the named core to `go-yjs` while preserving the substance of the principle: one core only, no reimplementation of CRDT logic, and a differential fuzz gate against real Yjs (which `go-yjs` supplies as a 13-surface oracle: 8 surfaces validated in BOTH directions, 5 — including sync and awareness — in direction A only).
...
- **Q**: What flush interval? → **A**: **Configuration, not specification.** It is an operational decision spanning at least ~500ms to ~10s; the service MUST expose it as a configurable knob and MUST NOT hard-code a value or assume a particular magnitude. The shipped default is 2s, matching the legacy whiteboard service. Shutdown flush is unconditional.
...
- **Q**: What is the acceptable write volume, given that each flush rewrites the entire document? → **A**: **Document the envelope; do not build adaptive behaviour.** Sustained write volume is approximately `document size ÷ flush interval × actively-edited documents`. That relationship MUST be documented alongside the flush-interval setting, and the shipped default MUST be defensible against the configured document-size limit rather than chosen arbitrarily.
  - The tension is **pre-existing, not introduced here**: the current model also rewrites the whole document. At the 2s legacy cadence and a 30 MiB document limit, one continuously edited document can still stage ~15 MiB/s. A longer configurable interval strictly improves the situation.
...
- **FR-022b**: Multi-pod fan-out together with a durable store MUST be REJECTED at startup, before the service begins serving. **Concretely**: `HUB_MODE=redis` with `CHECKPOINT_STORE=file-service` fails configuration load with an error naming both keys, the consequence, and the supported single-pod alternative. There is no override.
  - **Why rejection rather than a warning**: the configuration does not prevent the write that loses the data. There is no ownership, no fence, and the checkpoint store replaces rather than merges, so two pods that briefly diverge each write a complete state over the other's and the later writer silently discards edits it never received. That needs no unusual operator behaviour — two pods and one divergence. A "read-heavy deployment" is an expectation about traffic, not a read-only contract, and one write is enough.
  - **Nothing is lost by refusing it**: a single-pod deployment has the supported equivalent (`HUB_MODE=inmemory` with the same durable store). Multi-pod durable operation returns when an ownership mechanism does.
...
- **Yjs compatibility is a design guarantee of the library, not a risk this feature carries.** `go-yjs` is 100% compatible with Yjs by design — that is its reason to exist — and it is gated by its own differential oracle against real Yjs. That oracle covers 13 surfaces but is NOT bidirectional across all of them: 8 are validated in both directions, and 5 — undo, relative positions, sync, awareness, subdocs — only in direction A (real Yjs's bytes decoded by `go-yjs`). Encoding, merge semantics, and the V2 codec fall inside the 8, in both directions: the core's gate decodes `go-yjs`'s `EncodeStateAsUpdateV2` output with real Yjs and re-encodes it for comparison on every push, for updates, snapshots and GC'd updates alike. They are therefore **out of scope for verification here**, consistent with `002`'s treatment of the core as externally gated.
- **What the core's gate does NOT cover is `sync` and `awareness` in direction B** — the framing `go-yjs` *emits* is not differentially validated against real Yjs, and `backend/*` has no oracle coverage at all (it has conformance suites instead). This is not a hole in the exclusion above, because §IV already assigns transport framing, sync handshake sequencing and awareness handling to THIS service rather than to the core. It is covered here by concrete scenarios rather than by fuzzing: the JS-interop suite, in which real y-protocols clients decode this server's framing and report decode errors explicitly, plus the browser end-to-end run. Neither sweeps encoding edges.
- **One residual on the V2 exclusion, narrow and worth stating.** The skew is not between the core and one consumer — it is between ALL TESTING and ALL PRODUCTION. The core's direction-B gate and the Excalidraw fork's own suite both run a NEWER patch release of real Yjs than either production consumer (`server`, which decodes this service's durable blobs, and `client-web`) does. The fork's package declares Yjs as a *peer* dependency, so it ships none and the host supplies it; the production version satisfies the declared range, it is simply the end of that range nothing exercises. A patch-level V2 divergence between the tested build and the shipped one is therefore outside what any gate in the fleet measures. For the `go-yjs` v0.1.0 release run this pairing was checked once, concretely: a 651-byte bare V2 checkpoint written by this service was fetched from the live file-service, decoded with the consumer's own real-Yjs build via `ApplyUpdateV2`, and materialized the expected image element and file locator — with no inline `data:` payload — after which the browser reloaded and rendered it. That is ONE live scenario on ONE document. It is not fuzz coverage and must not be recorded as such.
