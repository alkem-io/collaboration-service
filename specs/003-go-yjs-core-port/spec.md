...
- **One residual on the V2 exclusion, narrow and worth stating.** The core's direction-B gate runs against a pinned real-Yjs version, while the consumer of this service's durable blob (`server`, via the Excalidraw fork) runs its own. A patch-level V2 divergence between those two is not what the gate measures. For the `go-yjs` v0.1.0 release run this pairing was checked once, concretely: a 651-byte bare V2 checkpoint written by this service was fetched from the live file-service, decoded with the consumer's own real-Yjs build via `ApplyUpdateV2`, and materialized the expected image element and file locator — with no inline `data:` payload — after which the browser reloaded and rendered it. That is ONE live scenario on ONE document. It is not fuzz coverage and must not be recorded as such.
