...
|---|---|---|---|
| `persistence.Store` | Durable append-only update log + checkpoints, with revisions, paged recovery reads, optional compaction and fencing | **No — contract only.** Decomposed into `Appender`/`Loader`/`Compactor`/`CompactingStore`, so compaction is opt-in | `MetadataStore` + `BlobStore` (whole-document snapshot + pointer) |
...
- **Q**: What does a flush write — incremental delta records, or the whole document? → **A**: **One whole-document blob per flush; one blob read on load.** The `Store` is checkpoint-only (`Appender` + `Loader` + `FenceMode`, deliberately **not** `Compactor`): `Load` returns a single `Checkpoint` with no trailing records and an empty `Next` — a complete, self-consistent view in one read. No record sequence is tracked and no compaction runs. The service's entire contract with the blob backend is **store blob, read blob** — nothing beyond that.
...
- **Q**: Should the backend-selection environment variable names stay as they are, given that one is named after a port this feature deletes? → **A**: **Rename them to match the adopted contracts.** The configuration vocabulary follows the contracts, not the superseded ports: the `persistence.Store` backing medium and the `hub.Hub` mode are named for what they now are. `METADATA_STORE` is unaffected — `MetadataStore` survives as a port in its own right (FR-009a).
  ...
  - **Critical hazard this creates**: an absent key used to fall back to a default — `BLOB_STORE` unset defaulted to `inline`, an in-process map. A stale manifest setting the *old* key after the rename is indistinguishable from one setting nothing, so the service would silently store blobs in memory, losing every document on restart while appearing healthy. **Resolved by making the canonical selectors MANDATORY** (FR-022f), not by recognising the abandoned names: `HUB_MODE` and `CHECKPOINT_STORE` have no defaults, so an omitted, misspelled, or renamed key fails startup naming the missing key and its supported values.
...
- **Q**: When a document is opened for the first time and has no saved snapshot yet, where should its initial content be loaded in? → **A**: **Inside the registry's open path.** The open function tries `persistence.Store` first and seeds from the content delivered by the metadata fetch only when nothing is stored. Because the registry coalesces concurrent cache misses into one call, first-open seeding is exactly-once **by construction** rather than guarded by a check that races.
  - This behaviour exists today (`room.seedFromContent`, and `006`'s `collaboration-fetch` content-for-seed field) and was absent from this spec; it is now explicit so the rebuild does not silently drop it.
...
- **Q**: Is the zero-dependency standalone mode a supported product configuration, or only a development/test convenience? → **A**: **Test/dev convenience only.** "Standalone" conflated two things: a product promise nobody wants, and in-process test fixtures that are genuinely needed. The promise is dropped (requires a §III amendment); the fixtures stay.
  ...
  - **Consequence for this feature**: exactly **one** `persistence.Store` is written, over file-service. No durable standalone `Store` is built. Tests use in-process fixtures and the shipped `InProcessRegistry` / `InProcess` hub. Auth, authz, and durable backends are not exercised by the fixture path.
...
- Two sessions open the same never-before-saved document simultaneously → exactly one seed occurs; the content is not applied twice. *[double-seed]*
- A document has stored content and a seed payload → the stored snapshot wins; the seed is used only when nothing is stored. *[seed precedence]*
...
- **FR-004a**: Document restoration MUST happen in the registry's open path, in this order: load from `persistence.Store`; if no snapshot is stored, seed from the content delivered by the metadata fetch. A session MUST NOT observe a partially initialised document.
- **FR-004b**: First-open seeding MUST be exactly-once per document even under concurrent opens, achieved by the registry's coalescing of cache misses rather than by an emptiness check performed after acquisition. Content MUST NOT be seeded twice.
...
- **FR-007**: Contract implementations MUST be native — written directly against the underlying infrastructure. The service MUST NOT implement a `go-yjs` contract by delegating to a superseded port (for example a `Store` that calls the old snapshot blob/pointer ports, or a `Hub` that wraps `ClusterBroadcaster`). Ports superseded by an adopted contract MUST be **removed, not wrapped**; no translation shim, compatibility layer, or adapter-over-adapter may survive (§VIII, §X).
- **FR-007a**: Choosing a shape the contract permits (such as a checkpoint-only store that does not implement `Compactor`) is conformant. **Misreporting a guarantee is not**: an `Append` returning success before the bytes are durable, a `Load` presenting partial history as complete, or a fan-out assuming ordering or single delivery the contract does not promise, are non-conforming regardless of how natively they are written or whether this repo's own tests pass.
- **FR-008**: Every custom port implementation MUST pass the library's corresponding conformance suites in CI.
- **FR-008a**: Where a persistence implementation CAN be fenced, it MUST be constructible in both fence modes and its fenced path MUST be exercised in CI — fence-capability that is built but never run does not satisfy this requirement. **Scoped by research.md D6a**: the file-service store cannot be fenced, because a fence epoch has nowhere durable to live on a file row and holding it in a separate service is explicitly not a substitute for a persistence-level backstop. It therefore reports `Unfenced` and relies on the cluster lease. This narrows the requirement to the in-process store rather than leaving it silently unmet.
...
- **FR-021**: The service MUST remain runnable in-process for tests without any external service, using in-process fixtures and the core's shipped single-process defaults. This is a **test capability, not a supported deployment mode** — no durable standalone `Store` is required or written.
...
- **FR-022d**: *(withdrawn — superseded by FR-022f.)* This required a renamed configuration key to fail startup naming its replacement, i.e. a per-key tombstone. Withdrawn as cost with no beneficiary for a service that has never shipped — that reasoning stands, and FR-022f does not reinstate it. The config still knows nothing about abandoned key names.
- **FR-022f**: The backend selectors `HUB_MODE` and `CHECKPOINT_STORE` MUST be explicitly set; there is no default for either. Startup MUST fail naming the missing key and its supported values.
  - **Why this rather than FR-022d**: a tombstone can only reject names it knows. This rejects every omission, typo, and abandoned name at once, because the failure is the ABSENCE of the canonical key rather than the presence of a retired one.
  - **Why it is required at all, given FR-022e**: FR-022e assumed the coordinated rollout would be complete. It was not — `specs/001/quickstart.md` was still exporting `FANOUT_MODE` and `BLOB_STORE` after the rename, and `os.Getenv` cannot distinguish a renamed key from an omitted one, so following that quickstart produced a deployment that believed it had redis fan-out and durable storage while running single-pod on the non-durable in-process store. An atomic-rollout requirement cannot be the only guard when an incomplete rollout is silent.
...
- **FR-022e**: *(necessary but NOT sufficient — see FR-022f, which exists because this requirement was not met in practice.)* The rename MUST land together with every consumer of the old keys — this repo's config, `.env.example`, and documentation; the k8s manifests on `feat/003-migration`; and `server`'s 006 `quickstart-services.yml`. A partial rollout would leave a consumer selecting a store that no longer exists under that name, which the silent default turns into non-durable storage — so the consumers land together rather than being guarded after the fact.
...
- **Recovery view** — the self-consistent read returned on load: one checkpoint, no trailing records, explicitly complete.
...
- **Ownership lease** *(deferred — not implemented in this feature)* — a time-bounded, fenced claim by one node over one document. Retained as a named concept because the persistence implementation is built fence-capable and its fencing path is tested (FR-008a), even though no deployment enables it.
...
- **SC-015**: A never-before-saved document opened concurrently by many sessions is seeded exactly once, with content identical to a single-session open.
...
- **SC-017**: The fencing conformance suite passes against a fenced instance of the store in CI, and the set of applicable suites is recorded with a reason for each suite not run.
...
- **SC-021**: *(withdrawn with FR-022d.)* Superseded by **SC-021a**: a process started with only the pre-rename keys set MUST fail startup; a process with both canonical selectors explicitly set MUST start. Both are asserted, and both fail if the selectors regain defaults.
...
- The service implements **exactly one** `persistence.Store` — over file-service — and one multi-pod `hub.Hub` over Redis. Writing these is the intended use of the library's extension points, and the conformance suites exist precisely to validate them. The core's shipped single-process defaults serve the in-process test path; no durable store is written for it.
- **A flush writes one whole-document update; a load reads one checkpoint.** *(Decided in Clarifications; mechanism corrected in research.md D1a.)* The `Store` is a genuine append log implementing `CompactingStore`, because the base conformance suite requires per-record fidelity and a paginated fixed recovery view that a latest-value store cannot satisfy. The **usage** is unchanged from the decision's intent: one whole-document update per flush window, compacted into the checkpoint immediately, so steady state is one checkpoint with no trailing records and recovery is a single read bounded by document size. FR-012 is now met **by compaction** rather than by the absence of history — the stronger guarantee, since an uncompacted conforming log would grow without bound.
...
- **Multi-pod ownership and fencing at runtime** — deferred; the persistence implementation is built fence-capable but runs unfenced. Consequently **durable multi-pod operation is out of scope for this feature**: fan-out is delivered and tested, single-writer durability is not, and the combination is documented as unsupported rather than silently shipped (FR-022a/b).
