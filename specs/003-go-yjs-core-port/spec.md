...
- **A flush writes one whole-document blob; a load reads one whole-document blob.** *(Decided — see Clarifications.)* The `Store` is checkpoint-only: it implements `Store` (`Appender` + `Loader` + `FenceMode`) and deliberately **not** `Compactor`/`CompactingStore`. No record sequence is tracked, no compaction runs, and recovery is a single read whose cost is bounded by document size rather than by edit history — satisfying FR-012 structurally rather than by policy.
