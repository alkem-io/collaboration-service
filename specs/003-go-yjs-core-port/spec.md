...
- **FR-008a**: The persistence implementation MUST be constructible in **both** fence modes, and the fencing conformance suite MUST pass against a fenced instance in CI — even though deployments run unfenced. Fence-capability that is built but never exercised does not satisfy this requirement.
- **FR-008b**: Suites that do not apply to the chosen shape are not required — notably the compaction suite, since the store deliberately does not implement `Compactor` (FR-007a). Which suites apply MUST be recorded explicitly, so an unrun suite is a stated decision rather than an oversight.
