...
- **FR-016** [Wave 3] — **SUPERSEDED** (by `003`, and by the ownership decision recorded in `contracts/unified-metadata-rmq.md`): the purge half no longer belongs to this service. Before `server` mutates the row, profile, bucket or blob it confirms a persistent `document.deleted` publish; `cmdCloseDeleted` then tombstones the id briefly, closes the room and evicts, and deliberately does not flush while `server` completes the owner cascade. `MetadataStore.Delete` no longer exists (the port is read/upsert only), and `BlobStore` — which WAS a real port here, introduced with the scaffold in `f4bc8bd` and removed in `45d8267` when `buildBlob` went — has been replaced by `persistence.CheckpointStore`, owned by the go-yjs core. Neither durable store is deleted by `document.deleted` any more. The surviving obligation is the temporary admission tombstone plus disconnect-and-release. Original text: *The server MUST consume the **`document.deleted`** lifecycle event and cascade — disconnect clients (`room-closed`), release the room, `MetadataStore.Delete` + `BlobStore.Delete` — idempotently, with no orphans (epic FR-012/FR-023; contracts/lifecycle-events.md; T015).*
...
- **SC-010** [Wave 3] — **SUPERSEDED**, same reason as FR-016: this service temporarily tombstones the id, disconnects clients and releases the room; it purges nothing. `server` confirms the event before starting its owner cascade, and "no orphan remains" is `server`'s guarantee to make. Original text: *A **`document.deleted`** event disconnects clients, releases the room, and purges metadata + blob **idempotently** — no orphan remains (epic FR-012/FR-023; T015).*
