...
- [X] **T004.4** [W2] `cmd/server` selects `redis` on `HUB_MODE=redis` (historical: `FANOUT_MODE`) from `REDIS_URL`; `inmemory` is now selected explicitly, not defaulted (003 FR-022f).
...
- [X] **T005.6** [W2] `cmd/server` selects metastore on `METADATA_STORE` (`inmemory` zero-dep default / `rabbitmq` / `postgres`) and the checkpoint store on `CHECKPOINT_STORE` (historical: `BLOB_STORE`); the chosen `BlobStoreKind` is persisted per metadata row and rehydration reads it back, so a doc loads from the right backend regardless of running config.
...
- [X] **T017.3** [W4] file-service blob-offload e2e (`test/e2e/fileservice_test.go`): boots with `CHECKPOINT_STORE=file-service` (historical: `BLOB_STORE`) against an in-process faithful stub file-service; persist→release→reload is identical, fetched back via the content pointer (metadata index inline, blob offloaded) (SC-006/SC-012).
